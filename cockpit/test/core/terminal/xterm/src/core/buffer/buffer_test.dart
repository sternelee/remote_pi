...
void main() {
  group('Buffer.getText()', () {
    ...
    test(
      'can handle line wrap', // ⚠ já falhava no fork/upstream (expectativa de wrap != comportamento shipped); skip documentado na absorção 2026-07-19
      skip: true,
      () {
        final terminal = Terminal();
    ...
    test(
      'can handle block range', // ⚠ já falhava no fork/upstream (expectativa de wrap != comportamento shipped); skip documentado na absorção 2026-07-19
      skip: true,
      () {
        final terminal = Terminal();
