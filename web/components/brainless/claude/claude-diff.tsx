  ...
  return (
    <div className={cn("min-w-0 font-mono text-[13px] leading-[1.55]", className)}>
      <div className="flex min-w-0 flex-wrap items-baseline gap-x-2">
        ...
        <span className="text-[var(--pi-fg)]">Update</span>
        <span className="min-w-0 break-all">
          <span className="text-[var(--pi-dim)]">(</span>
          <span className="text-[var(--pi-cyan)]">{file}</span>
          <span className="text-[var(--pi-dim)]">)</span>
        </span>
      </div>
      {summary ? (
        <div className="flex min-w-0 items-baseline gap-2 text-[var(--pi-muted)]">
          {/* invisible status glyph spacer: aligns ⎿ under "Update" */}
          <span aria-hidden className="invisible shrink-0">
            ⏺
          </span>
          <span aria-hidden className="shrink-0" style={{ color: "var(--pi-dim)" }}>
            ⎿
          </span>
      ...
      <pre className="mt-1 min-w-0 overflow-x-auto rounded-none border border-[var(--pi-border-soft)] bg-[var(--pi-surface)] py-1.5 pl-2 pr-3">
        {lines.map((l, i) => {
          const bg =
            l.type === "add"
              ? "rgba(78, 169, 111,.10)"
          ...
          return (
            <div key={i} className="flex min-w-0" style={{ background: bg }}>
              ...
              <span
                ...
                style={{
                  color: l.type === "ctx" ? "var(--pi-muted)" : "var(--pi-fg)",
                }}
              >
                {l.type !== "ctx" ? (
