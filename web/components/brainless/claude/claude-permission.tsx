...
export function ClaudePermission({
  ...
  return (
    ...
    >
      ...
      <div className="text-[var(--pi-fg)]">{command}</div>
      <div className="mb-1.5 mt-2 text-[var(--pi-fg)]">{question}</div>
        ...
        {options.map((opt, i) => {
          ...
          return (
            <div
              ...
              className="flex cursor-pointer items-baseline gap-2 rounded px-1 py-0.5 outline-none focus-visible:ring-1 focus-visible:ring-[var(--pi-cyan)]/60"
              style={{ background: active ? "color-mix(in srgb, var(--pi-rose) 12%, transparent)" : "transparent" }}
            >
              <span
                aria-hidden
              ...
              <span
                style={{ color: active ? "var(--pi-fg)" : "var(--pi-muted)" }}
