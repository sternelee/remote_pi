...
const STATUS_COLOR: Record<Status, string> = {
  success: "var(--pi-green)",
  error: "var(--pi-red)",
  pending: "var(--pi-yellow)",
};
    ...
    >
      <summary
        className={cn(
          ...
          "rounded-none outline-none focus-visible:ring-1 focus-visible:ring-[var(--pi-cyan)]/60",
        )}
      >
        <span className="flex min-w-0 items-baseline gap-2">
          ...
          <span className="min-w-0 break-words">
            <span className="text-[var(--pi-fg)]">{tool}</span>
            {arg !== undefined ? (
              <>
                <span className="text-[var(--pi-dim)]">(</span>
                <span className="text-[var(--pi-cyan)]">{arg}</span>
                <span className="text-[var(--pi-dim)]">)</span>
              </>
            ) : null}
          </span>
        </span>
        <span className="flex min-w-0 items-baseline gap-2 text-[var(--pi-muted)]">
          {/* invisible status glyph spacer: aligns ⎿ under the tool name */}
          <span aria-hidden className="invisible shrink-0">
            ⏺
          </span>
          <span className="flex min-w-0 items-baseline gap-2">
            <span aria-hidden className="shrink-0 text-[var(--pi-dim)]">
              ⎿
            </span>
            <span className="min-w-0 break-words">
              {result}
              {expandable ? (
                <span className="ml-2 text-[var(--pi-dim)] group-open:hidden">
                  (ctrl+o to expand)
                </span>
              ) : null}
            </span>
      ...
      {expandable ? (
        <div className="mt-1 whitespace-pre-wrap pl-[32px] text-[var(--pi-muted)]">
          {children}
        </div>
      ) : null}
    </details>
